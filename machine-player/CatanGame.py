...
class CatanGame:
  ...
  def reset(self):
    ...
    tile_types = ["ore", "wool", "lumber", "grain", "brick", "wool", "brick", "grain", "lumber", "desert", "lumber", "ore", "lumber", "ore", "grain", "wool", "brick", "grain", "wool"]
    # Create the tiles
    tiles = []
    for i in range(19):
      temp_tile = Tile(tile_types[i], i, False, None, None, None, None, None, None, None, None, None, None, None, None, tile_values[i], None, None, None)
  ...
  def step(self, action):
    ...
    # If the action is legal, is the action a 4:1 trade?
      ...
      self.resource_pool[action_parts[5]] = self.resource_pool[action_parts[5]] - 4
      # Increase the received resource by 1
      self.resource_pool[action_parts[6]] = self.resource_pool[action_parts[6]] + 1
    ...
    # If the action is legal, is it a road build?
      ...
      self.board.build_road(action_parts[2], action_parts[3], action_parts[4], action_parts[5])
    ...
    # If the action is legal, is it a settlement build?
      ...
      self.board.build_settlement(action_parts[2], action_parts[3], action_parts[4], action_parts[5])
  ...
  def set_legal_actions(self):
    ...
    for action in self.all_actions:
      ...
      if ((action_parts[0] == "build" and action_parts[1] == "settlement") and (self.resource_pool["brick"] < 1 or self.resource_pool["wool"] < 1 or self.resource_pool["lumber"] < 1 or self.resource_pool["grain"] < 1)):
        continue
      ...
      elif ((action_parts[0] == "build" and action_parts[1] == "road") and (self.resource_pool["brick"] < 1 or self.resource_pool["lumber"] < 1)):
        continue
      # If the location already has a settlement on it, skip this action; consider it cut from the list of legal actions
        ...
        if vert_val is not None:
          ...
          settlement_legal = self.board.check_settlement_placement_legal(q_coord, r_coord, s_coord, direction)
      ...
      # If the location already has a road on it, skip this action; consider it cut from the list of legal actions
        ...
        if side_val is not None:
          ...
          road_legal = self.board.check_road_placement_legal(q_coord, r_coord, s_coord, direction)
          if road_legal == False:
            continue
          else:
            self.legal_actions.append(action)
      # If the player tries to make a 4:1 trade with the bank, check if they have enough resources to make the trade
      elif (action_parts[0] == "trade" and action_parts[1] == "bank" and action_parts[2] == "4" and action_parts[3] == "for" and action_parts[4] == "1"):
  ...
  def set_all_possible_actions(self):
    ...
    for tile in self.board.get_board_tiles():
      ...
      self.all_actions.append(f"build_settlement_northwest_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_settlement_north_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_settlement_northeast_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_settlement_southeast_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_settlement_south_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_settlement_southwest_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_road_northwest_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_road_northeast_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_road_east_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_road_southeast_{q_coord}_{r_coord}_{s_coord}")
      self.all_actions.append(f"build_road_southwest_{q_coord}_{r_coord}_{s_coord}")
